...
function process(){
	if(jytj&&jytj.list){
		...
		for(var i=0;i<data.length;i++){
			trs+='<tr><td class="title title-dot"><a href="jytj_detail.html?p=json/jytjdtl/'+data[i].jsonpath+'">'+data[i].Title+'</a></td>';
			trs+='<td class="date">'+data[i].time+'</td></tr>';
		}
		var pages=pager(page_NO,jytj.pages-1);
		...
		trs+='<span><a href="'+filename+'?p='+(page_NO>=jytj.pages-1?jytj.pages-1:page_NO-0+1)+'" target="_self">&gt;</a></span></div></th></tr>';
		table+=trs+"</table>";
		$("#jytj_ul").html(table);$("#jytj_ul").removeClass("hide");
	}
}
