...
function process(){
	if(gg&&gg.list){
		...
		for(var i=0;i<data.length;i++){
			var url=data[i].pUrl;
			if(data[i].PUB_TYPE=='NSSGSGG'&&data[i].pUrl2nd&&data[i].pUrl2nd.indexOf(".js")!=-1){
				url="gsgg_detail.html?p=json/gsggdtl"+data[i].pUrl2nd;
			}
			trs+='<tr><td class="title title-dot"><a href="'+url+'">'+data[i].Title+'</a></td>';
		...
		table+=trs+"</table>";
		$("#nssgsgg_ul").html(table);$("#nssgsgg_ul").removeClass("hide");
	}
}
