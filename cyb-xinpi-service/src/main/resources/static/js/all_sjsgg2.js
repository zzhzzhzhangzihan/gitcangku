...
$(function () {
    getsjsgg2bulltein(1); //加载交易所公告数据
});
//最新公告
function getsjsgg2bulltein(pageindex) {
    var url = "/cyb/v1/sjsgg/"+pageindex+".json"
    $.ajax({
        url: url,
        type: "GET",
        dataType: "json",
        ...
        success: function (json) {
            if (json != null && json.data.data.length > 0) {
                ...
                trs+="</table>";
                trs += p_i_sjsgg(pageindex, json.data.total);
                $("#gsgg_ul").html(trs);
                init_sjsgg(pageindex,json.data.total)
            } else {
...
function init_sjsgg() {
    ...
    $("#go_1").click(function () {
        ...
        getsjsgg2bulltein(index);
        return false;
    });
}
function p_i_sjsgg(page, total) {
    var retunt = "";
