...
function process(){
	if(jyzl&&jyzl.list){
		...
		for(var i=0;i<data.length;i++){
			trs+='<tr><td class="title title-dot"><a href="sczm_detail.html?p='+data[i].jsonpath+'">'+data[i].Title+'</a></td>';
			trs+='<td class="date">'+data[i].time+'</td></tr>';
		}
		var pages=pager(page_NO,jyzl.pages-1);
		...
		trs+='<span><a href="'+filename+'?p='+(page_NO>=jyzl.pages-1?jyzl.pages-1:page_NO-0+1)+'" target="_self">&gt;</a></span></div></th></tr>';
		table+=trs+"</table>";
		$("#jyzl_ul").html(table);$("#jyzl_ul").removeClass("hide");
	}
}
